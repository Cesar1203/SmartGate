...
export const orders = pgTable("orders", {
  ...
  departureTime: timestamp("departure_time").notNull(),
  destination: text("destination").notNull(),
  requestedProducts: jsonb("requested_products").$type<ProductQuantities>().notNull(),
  status: text("status").notNull().$type<OrderStatus>().default("pending"),
  reliability: real("reliability"), // percentage 0-100
  weatherData: jsonb("weather_data").$type<WeatherData>(),
  recommendations: text("recommendations"),
  adjustedProducts: jsonb("adjusted_products").$type<ProductQuantities>(),
  timestamp: timestamp("timestamp").notNull().defaultNow(),
});
...
export interface OrderRecommendation {
  ...
  message: string;
  suggestedProducts: ProductQuantities;
